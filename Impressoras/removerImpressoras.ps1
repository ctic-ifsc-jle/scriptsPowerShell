...
'running with full privileges'
 ...
 [string[]] $impressoras = 'Microsoft XPS Document Writer','Microsoft Print to PDF','Fax','OneNot';
...
foreach ($temp in $impComputador) {
    ...
    $temp=$temp.Trim("@{Name=;}");
    if( $temp -notin $impressoras){ 
        cscript "C:\Windows\System32\Printing_Admin_Scripts\pt-BR\prnmngr.vbs" -d -p $temp
        $temp + " foi removida";  
    }else{ 
